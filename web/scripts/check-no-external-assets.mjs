...
const ALLOWED_HOSTS = [
  ...
  // 官网（site/dist）里指向我们自己控制面的**导航**链接：用户点了才跳，不是被取的资源。
  // 🔴 撤销条件：哪天官网真的去 fetch 会员中心（比如首页显示配额），这两条必须先删掉再讨论 ——
  //    那时它就成了一个跨域请求，而官网的 CSP 是 `connect-src 'none'`。
  ['babel.plus', '官网页脚与按钮指向自家控制面的导航链接（web./api. 同域下的子域一并覆盖）'],
...
const FETCH_VECTORS = [
  ...
  // 🔴 只管 `<link href>`（样式表 / preload —— 这些**会被取**），**不管 `<a href>`**。
  // 一个 `<a href>` 是导航，不是资源：用户点了才走，而官网指向会员中心的链接正是产品本身。
  // 2026-09-04 修：官网上线时这条规则把三条自家导航链接判成违规。
  // 放宽的只是「取」与「跳」的区别 —— 外部主机名仍然要过第 ③ 层的登记，一个都逃不掉。
  [/<link[^>]+\shref\s*=\s*["']https?:\/\/[^"']+["']/gi, 'HTML link href'],
