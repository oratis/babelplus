...
2. ~~**不证明外网 IP 到底按哪条 SKU 计。**~~ ✅ **2026-09-05 已用账单实数核实（§4.1）**：走 `External IP Charge on a Standard VM`，$11/月口径成立。
...
### 4.1 ✅ 2026-09-05 实测定论（roadmap B71 关闭）

上面那条 BigQuery 于 2026-09-05 07:1x UTC 跑过一次（`bq query`，身份 `wangharp@gmail.com`，窗口 2026-08-01 → 09-04，35 天）：

| SKU | `usage.unit` | 用量 | 实收 |
|---|---|---|---|
| **`External IP Charge on a Standard VM`** | **seconds** | 6,316,372 s ≈ **1,754.5 h** | **$5.87** |
| `Static Ip Charge in Hong Kong` | seconds | 2,483 s | $0.00 |
| `Static Ip Charge` | seconds | 531 s | $0.00 |

**结论：挂在运行中实例上的保留静态 IP 走第一条 SKU**（720 h/月免费档之后 $0.005/h）。
1,754.5 h 与「`vpn-us` + `vpn-jp` 两个 IP × 35 天 + `bp-node-hk1` 自 09-01 起」的量级一致；
后两条 `Static Ip Charge*` 只出现在 bp-node-hk1 建机期间（候选地址短暂预留），且计 $0。
**预算表按 $11/月（4 个 IP ≈ 2,920 h − 720 h 免费 = 2,200 h × $0.005）记是对的**；$29–44/月那条候选可以划掉。
⚠️ `usage.amount` 的单位是**秒**不是小时——直接读表会把用量放大 3,600 倍。
