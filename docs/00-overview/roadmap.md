...
| B72 | ~~**自用机队的订阅域名未定（D5）**~~ ✅ **2026-09-05 关闭**：用户指定 `sub.telloria.com`（独立 zone，不与 `gptwiki.net` 的 CDN 保险通路同失效面）；Worker `custom_domain` 绑定，`curl` 验头 200，订阅产物与三台节点上报地址全部切换 | 订阅热更新有了大陆可用的入口（**可达性需真机**：本机在代理后测不出） | ✅ 已关闭；真机加载 Clash Verge Rev / Shadowrocket 仍未做 |
