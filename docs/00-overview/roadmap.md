...
- [ ] 🔶 **落地页已建成并部署，但 DNS 未切**（2026-09-04）：`web/site/`（Vite，**产出零 JavaScript**，
      ...
      ✅ **2026-09-04 已上线**：用户裁决改指 apex（原先指向 Vercel 上一个在线的无关产品
      「LLM Game Platform」，已因此下线；回滚值与 RecordId 记在 first-deploy §4.6）。
      `https://babel.plus/` 与 `https://www.babel.plus/` 实测 **200**，证书是 Let's Encrypt
      （一张证书四个 SAN，到期 2026-12-03）。
      ⚠️ 仍缺：`docs.*` 教程站不存在；站点上没有可下载的客户端（两个都还不能分发）；
      自助购买未开（F8），页面据此显示「暂不开放自助购买」。
...
| B68 | 🔴 **babel.plus 的节点切换现在没有回滚落点**（2026-09-04 新增）。[ADR 0007 §4/§8](../05-adr/0007-node-migration.md) 原本把 `vpn-jp` 定为「第一次节点切换的人工回滚落点」，而 [ADR 0017](../05-adr/0017-personal-fleet-in-repo.md) **撤销了这一条** —— 真走那条回滚 = 让付费用户的流量落到用户自用的机器上，与 2026-09-04 的隔离指示直接冲突，且 `vpn-us` 已实测撞 CPU 天花板（29 Mbps）。**这个缺口此前被一条不该存在的依赖掩盖着，现在它显式了** | ADR 0007 阶段 3→4 的任何一次节点切换。当前 `bp-node-*` **只有 1 台**，所以「切过去、发现不行、切回来」这条路径**不存在** | 需建第二台 `bp-node-*`（**不是**自用队的机器）
| B69 | **`infra/scripts/verify-isolation.sh` 把两台自用机的 zone/机型/IP/RUNNING 写死成硬期望**（2026-09-04 新增）。[ADR 0017 §3](../05-adr/0017-personal-fleet-in-repo.md) 要扩容自用队到 4 台，**加第三台就会让它误报** —— 而误报的必然结局是有人把它改宽松，那一刻两支机队的隔离就名存实亡。两条扩展：① 期望清单改读 `infra/fleet/fleet.json`；② 加反向断言（`bp-*` 不得带 `vpn-node` 标签，反之亦然，两组防火墙 target tag 不得交叉） | 隔离保证本身。**这是建 `vpn-sg` 的前置，不能并行**<br>🔴 **2026-09-05 追记：它在 2026-09-04 21:15 就已经红了**——用户跑 `optimize-vpn.sh` p1 加了 `vpn-deny-from-bp`，非 bp- 规则 11 ≠ 硬期望 10，而 `deploy.yml` 的两个部署作业都 `needs: isolation-before`（潜伏：工作流是手动触发，没有失败记录） | ✅ **2026-09-05 已做**：期望改读 `infra/fleet/fleet.json`（D7 入库），加两条反向断言 + 一条正向断言（守 `vpn-deny-from-bp`），本地 23/23 绿
| B70 | 🔴 **自用队的 443 入向挂在没有 target tag 的规则上**（2026-09-04 实查新增）。`allow-xray-443` / `allow-hysteria-udp443` 无 tag，而 [as-built-gcp §3](../02-architecture/as-built-gcp.md) 的处置建议本身就是「给它们补 tag 做收敛」。`bp-node` 有自己的 `bp-allow-*-443` 所以免疫，**`vpn-*` 没有** —— 那次收敛会让两台自用机瞬时失去 443（六条通路断四条），而现象与 IP 级封锁的三条取证特征完全吻合，排障会走到「释放 IP、重开机器」上去 | 那条**正确的**安全收敛动作现在有副作用，于是没人敢做 | ✅ **2026-09-05 已建** `vpn-allow-reality-443` / `vpn-allow-hy2-udp443`（prio 1000，target `vpn-node`，纯增量）；`fleet.json` 的期望清单同步到 13 条
| B71 | **外网 IPv4 的计费口径未定**（2026-09-04 新增）。[价目 §4](../evidence/fleet-pricing-20260904/) 实测目录里有两条候选 SKU：`External IP Charge on a Standard VM`（720 h/月免费后 $0.005/h）与 `Static Ip Charge`（$0.010–0.015/h），**差 3–4 倍**（$11/月 vs $29–44/月），从目录本身判不出挂在运行中实例上的保留静态 IP 该走哪条。⚠️ `VPN方案设计.md` §二 写的「静态 IP 挂在运行中 VM 上免费」**已过时** | 机队的固定成本基线，进而影响 $500 上限下的出口余量 | ✅ **2026-09-05 已定论**（[价目 §4.1](../evidence/fleet-pricing-20260904/)）：走 `External IP Charge on a Standard VM`（`usage.unit` = **seconds**），35 天实收 $5.87，$11/月口径成立，$29–44 那条划掉
| B72 | **自用机队的订阅域名未定（D5，2026-09-05 新增）**。订阅 Worker `fleet-sub` 已上线但只有 workers.dev 地址（大陆不可达，只当发布验证）；裁决要求独立 zone（不挂 `gptwiki.net`——与 CDN 保险通路同失效面；不挂 `babel.plus`） | 三台设备的订阅热更新在大陆实际不可用；公告伪节点名里也写不出域名 | **需用户**给一个独立 zone；之后 `wrangler.jsonc` 加 routes → `fleet.json .subscription.hostname` → publish → 重装 healthcheck env（runbook §5） |
| B73 | **飞书日报的收件会话未定（D3，2026-09-05 新增；同日用户改为用应用「胖猫」）**。Worker cron、卡片渲染、应用鉴权均已实测，但胖猫只在 16 个有其他人的群里，没有只有用户本人的会话；日报含节点 IP，**收件会话确定前不发** | 日报是 $500 上限的唯一执行点（ADR 0017 §6），现在只有观测没有投递 | **需用户**建一个只有本人 + 胖猫的群（或给飞书邮箱/手机号取 `open_id`），然后 `wrangler secret put FEISHU_RECEIVE_ID`，`daily-report.py --send` |
