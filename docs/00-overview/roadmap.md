...
| B73 | **飞书日报的 webhook 未取得（D3，2026-09-05 新增）**。Worker cron 与卡片渲染已实测（`GET /admin/report` 绿卡），但发送路径一次都没跑 | 日报是 $500 上限的唯一执行点（ADR 0017 §6），现在只有观测没有投递 | **需用户**建「只有本人 + 自定义机器人」的群、开签名校验，把 URL / secret 交给 `wrangler secret put`，再 `daily-report.py --send` |
