...
| B73 | ~~**飞书日报的收件会话未定**~~ ✅ **2026-09-05 15:53 关闭**：用户改用应用「胖猫」；`feishu-notify.sh --whoami` 按手机号反查到本人 `open_id`，Worker 经 `im/v1/messages` 私聊发出首条真实日报（code 0），cron 每日 08:37 CST。日报含节点 IP，只发私聊、不进任何群 | 日报是 $500 上限的唯一执行点（ADR 0017 §6），现在观测与投递都通了 | ✅ 已关闭；剩 App Secret 重置（对话中明文出现过） |
