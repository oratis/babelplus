...
> [ADR 0011](../05-adr/0011-domain-blackout-detection.md)（§14 的域名池与模块 17）、
> [ADR 0012](../05-adr/0012-payment-gateway.md)（**推翻 §2.6 / §5.1 / §8.1 的「小地址池 + 金额尾数匹配」**，落点见 §8.1）、
> [ADR 0013](../05-adr/0013-billing-and-refund-rules.md)（§14 的折抵算法与流量包重置）
> —— **三份均为提案，未批准**（2026-08-23）
> **2026-08-29 补登**：§8.1 与 §14 各加落点，端点、字段与状态机一处未改。
...
> **2026-08-29 补登落点：支付网关已裁决（roadmap B21）—— [ADR 0012](../05-adr/0012-payment-gateway.md)（提案，未批准）。**
>
> **裁决：自扫链 + 一单一址永不复用 + 第一阶段一次都不归集**（0012 §1、§5.1、§13.1）。
> 不接易支付聚合（0012 §3.2：虎皮椒协议 4.10.10 / 4.10.17 点名拒收本类目，且签约主体必须是境内自然人），
> 也不用 EPUSDT 自托管进程（0012 §3.1：它要自己管理收款地址，等于在服务器上跑一个能动钱的服务）。
>
> 🔴 **本文写的「金额唯一性匹配」被 0012 §3.6 显式推翻，逐处登记如下**（本文今天不改写，因为 0012 未获批准）：
>
> | 本文位置 | 现文 | 0012 的处置 |
> |---|---|---|
> | **§8.1 上图第 10 步** | `金额唯一性占位 / 冲突则 +100 usdt6 重试(最多100次)` | 🔴 **推翻**。改为 0012 §5.1 的分配算法：**订单与收款地址一一对应，地址永不复用**；归属只看 `to_address`，金额只用于判定 `paid`/`underpaid` |
> | **§2.6 数值单位表**「USDT 金额」行的理由列 | 「金额唯一性匹配的 `+0.0001` 递增正好是 **+100**」 | 🔴 **理由推翻，单位保留**。`*_amount_usdt6` 仍是整数、单位 1e-6 USDT（0012 §3.6 明写「零处 schema 形状变更」），但 `+100` 这个论据随尾数机制一起消失。报价改为**取整到 0.01 USDT**（0012 §5.3） |
> | **§5.1 收银台 JSON 的 `amount_display`** | `"5.8423"`（四位小数） | 🔴 **推翻**。改两位小数字符串（如 `"55.60"`），且它是**展示用字符串，不是数值类型**（0012 §3.6） |
> | **§5.1 收银台 JSON 的 `note`** | 「尾数 0.0023 是订单识别码，请按此金额一分不差地转账」 | 🔴 **推翻**。改为解释**提币手续费从转出额扣除**、并给出含手续费的填写金额（0012 §6.4）。同处推翻 [user-journey](../03-product/user-journey.md) §7 卡点 5 的原文案 —— 原文案对 Binance 式提币是**反的**，照它填必然 `underpaid`（0012 §3.7） |
> | **§5.1 三条对应事实的第 1 条** | 「`amount_usdt6` 的末位是订单识别码……抄 EPUSDT：冲突则 `+0.0001` 递增重试，最多 100 次」 | 🔴 **整条推翻**，连同它引用的 `pricing-and-plans` §4.2 裁定 |
> | **§5.1 三条对应事实的第 2 条** | 「**金额唯一性匹配决定了少付一定会发生**（提币手续费从转出额扣是头号成因）」 | ⚠️ **结论保留，理由更换**：`state` 仍**必须**含 `underpaid`，但成因改写成「提币手续费从转出额扣决定了少付一定会发生」。枚举取值 `waiting`/`confirming`/`underpaid`/`paid`/`expired` 与 `shortfall_usdt6` **一个都不动**（0012 §3.6） |
>
> **本文之外的连带落点（本次不改，只登记）**：`openapi/openapi.yaml` 六处 description 被 0012 §3.6 改写
> （**六处全部是 description，零处 schema 形状变更**）；`orders_pay_addr_amount_uk` 改为 `orders_pay_addr_uk`（0012 §17.2）。
> 🔴 **本节今天不改写**：0012 状态是**提案，未批准**（2026-08-23）。
...
- [ ] **升级折抵算法未定。** user-journey 只定了呈现口径（三行：原套餐剩余价值 / 新套餐价 / 实付），
      ...
      > **2026-08-29 补登落点：[ADR 0013](../05-adr/0013-billing-and-refund-rules.md) 裁决 ②（roadmap B25，提案，未批准）。**
      > **算法定了：只按剩余天数折抵金额，配额按本周期剩余天数折算发放**（0013 §4.2 给公式、§4.4 给配额口径），
      > 折抵基数 `V_source = amount_paid + amount_balance + surplus_amount`，**折抵不产生找零**，
      > **禁止中途降级**（§4.3 边界 1 → `422 DOWNGRADE_NOT_ALLOWED`），升级单不收优惠码。
      > 0013 §1.2 明写 `payments` §4.6 的 `min(剩余天数比, 剩余流量比)` **从来不是一条成形的建议**
      > （两个比例量纲不同，在长周期订单 + 月度重置的默认路径上会给出 **0 折抵**）。
      > **本文缺的那段契约**由 0013 §4.6 补上：`orders.type` 改为**服务端推导**，不由客户端传。
      > ⚠️ 0013 §4.7 另登记 `POST /orders` / `GET /plans` 上 **OpenAPI 与 DB 枚举的四处不一致**，上线前必须修。
      > 🔴 **本条不划掉**：0013 状态是**提案，未批准**（2026-08-23），且它依赖迁移 `0016`（0013 §6）。
- [ ] **流量包与 `reset_traffic_method` 的相互作用未对齐。** user-journey 建议「周期重置时保留流量包配额」，
      ...
      > **2026-08-29 补登落点：[ADR 0013](../05-adr/0013-billing-and-refund-rules.md) 裁决 ③（roadmap B26，提案，未批准）。**
      > **裁决：保留（跨周期结转）**，即采纳 user-journey §10.1 的建议。
      > `users.transfer_enable` 拆成 `transfer_enable_plan` + `transfer_enable_pack`，
      > `transfer_enable` 改为二者之和的 **`STORED NOT NULL` 生成列**（0013 §5.2）；
      > 消耗顺序**先套餐后加油包**、加油包 **12 个月有效**（0013 §5.3、§5.5）；
      > 与 `reset_traffic_method` 的对齐见 0013 §5.4。
      > 生成列的理由是 D1「管理员直接改配额」这条**绕过应用层的人工写路径存在**，
      > 只有 schema 层才能让「两个分量与总额不一致」不可表达。
      > 🔴 **本条不划掉**：0013 状态是**提案，未批准**（2026-08-23）。
- [ ] **域名池与可达性探活的 API（管理面模块 17）没有设计。**
      ...
      > 洞实际是**七处**不是三处（0011 文档头列全），本条是其中之一。
      > 0011 §14 给本文两处具体增量：**§6.1 模块 17** 在现有 `GET/POST /admin/domains` 与 `DELETE` 之外补
      > `PATCH /admin/domains/{id}`（改 state / order）、`POST /admin/domains/{id}/mark-blocked`（人工置位，D17）、
      > `POST /admin/domains/{id}/promote`（启用冷备，D17，L1 确认串 + L2 原因 + L3 TOTP）；
      > **§4.4** 的 `profile-update-interval: 24` 改 `2`（0011 §4.4，理由已换成探测腿的采样率）。
      > ⚠️ 0011 同时要求 `openapi/openapi.yaml` 新增 `GET /-/probe` → 204 且 `security: []`，
      > 并把 `/-/healthz` 的返回写准为纯文本 `ok bp-api` —— **不属本文范围，只登记**。
      > 「≤ 30 分钟」本身被 0011 §12.3 拆成四个口径，见 [user-journey](../03-product/user-journey.md) §16。
