...
| **2 · 建节点** | 预留 `bp-node-hk1-ip` → 建 VM（`--tags=bp-node`）→ 装机 | ① IAP SSH 通、公网 22 **拒绝**；② `tcp/udp:443` 从境外可达；③ **三网路由实测合格**（ADR 0004 §3.5）；④ 证书链签发者是 **Let's Encrypt 不是 GTS**（ADR 0004 §3.4）；⑤ **该 `node_id` 的 `bp_node_alive` time series 已出现，A2/B1 已确认 armed**（ADR 0014 §14 增补，2026-09-02 批准后落下；metric absence 要求序列曾有数据，写成验收项它才是一个门） | 释放 IP 重开；或整机删除 | 零用户影响（无用户） |
