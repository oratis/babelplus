...
| [0010](0010-domain-strategy.md) | 按故障域买五个中性主域名，品牌不进域名；NS 全放注册商，CF 在 P1 持零个池内 zone —— 消解 system-design §2 与 §4.1 的矛盾 | **提案，未批准**（2026-08-23）·⚠️ 含一笔不可退采购。`babel.plus` 归属已答（2026-08-25：项目所有者自己的，生产 CORS 不动） |
...
> **本目录当前有 14 份 ADR：`0001`–`0008` 与 `0010`–`0015`。`0009` 是空号，不是丢文件。**
> 2026-08-29 `ls docs/05-adr/` 实数复核，上表逐行与文件名对齐。
