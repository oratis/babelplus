...
> [ADR 0014](../05-adr/0014-slo-and-oncall.md)（**推翻 §4 的「不做分级」，把 §5 的 17 条改成 A/B/C 三档**；
> 并裁决 §14 登记的 SLO 与 on-call 两条缺口。**已批准**（2026-09-02；原提案 2026-08-23）—— 落点见 §4、§5、§14）
> **2026-08-29 补登**：§4、§5、§14 各加落点，本文的通道拓扑、17 条阈值与 §12 值班顺序**一处未改**
> —— 0014 未获批准之前，实际要建的仍是本文写的东西。
> **2026-09-02 补登**：0014 已批准并首轮实施。**实际建成的告警策略以 `infra/alerts/*.json` 为准**
> （`setup-alerts.sh --emit-json` 的入库产物），§4 / §5 的原表保留为设计稿，不再是实况。
> 实况清单见 [first-deploy §4.5](first-deploy-20260831.md)。
...
> | `bp_node_alive` | ⚠️ **应用侧已就绪（2026-08-23），指标仍未创建** | `handler/nodealive.go` 已开始写日志（文案就是指标名、带字符串 `node_id`、每节点每 60 秒最多一条）。**剩下的一半是在 GCP 上 `gcloud logging metrics create` 并配 `--label-extractors`**，见下方命令。**§5 的 metric-absence 告警依赖它，节点上线前必须先有** |
...
> **2026-08-29 补登落点：本节「两条都挂在每一条策略上，不做分级」这句被
> [ADR 0014](../05-adr/0014-slo-and-oncall.md) §8 推翻（roadmap B34/B35，提案，未批准）。**
>
> 0014 §8.2 承认**这句话在 2026-08-16 是合理的**（那时一条策略都没有，先有比没有好），
> 并给出今天必须改的三条理由：
>
> 1. **17 条不分级 = 17 条都有权在凌晨三点叫醒一个人**，而其中至少 9 条在凌晨三点没有任何可执行动作。
...
>    （与本节上面那条 2026-08-21 实查一致）。**今天的实况是「单通道 + 一个假的冗余」。**
> 3. **省的不是钱，是注意力**：0014 §8.2 算过，瘦身后每月只省 **$1.40**（$5.95 → $4.55，
>    费率与生效日期 **待核实**）。
>
> **改成什么**（0014 §8.1 的档位表 + §8.3 的通道矩阵）：
>
> | 档 | 判定 | 条数 | 通道 |
> |---|---|---|---|
> | **A** | 用户此刻正在掉线，或存在安全/资金的持续损害；**且至少两条 A 级来自不同故障域** | **3 条**（2 条带外 + 1 条 GCP） | email#1 + **email#2（只接 A 级的语义隔离箱）** + 原生推送 + Pub/Sub 归档 |
> | **B** | 有真实问题，但拖到工作时间不会让损害扩大 | **11 条** GCP 策略 | email#1 单通道 |
> | **C** | 趋势或容量信号，没有单点处置动作 | **0 条策略** —— 只进看板与月巡检 | 无 |
>
> 划档依据不是「严重程度」这种主观词，而是两个可判定的问题（0014 §8.1），**问题一优先**：
> 「这条告警在我们的基础设施整体失效时还工作吗？」不工作的，**不能是 A 级的唯一来源**。
> 因此 A 级 3 条里有 2 条**跑在 GCP 之外**（Uptime Kuma 与带外 VPS cron）。
> **「Pub/Sub 是主通道」也被改写**：0014 §14 把它降为**事后取证**通道（另建 pull 订阅 `bp-alerts-archive`），
> 现阶段不算送达通道。静默纪律见 0014 §8.4。
> 🔴 **本节今天不改写**：0014 状态是**提案，未批准**（2026-08-23）。
> **2026-09-02：0014 已批准。** 本节仍不重写（追加不抹），但从这一天起**实况是 0014 §8.3 的通道矩阵**：
> B 级 → email#1 单通道，已建；A 级四通道**未齐**（email#2 / 推送未建），A 级策略一条未建。
...
> **2026-08-29 补登落点：上表这 17 条被 [ADR 0014](../05-adr/0014-slo-and-oncall.md) §8/§9 重新分级
> （提案，未批准）—— 17 条不分级 ⇒ A 级 3 条 + B 级 11 条 GCP 策略 + C 级不建策略，`bp-*` 合计 14 条。**
>
> - **A 级 3 条**（0014 §9.1），**其中 2 条不在 GCP**：`A1` 全部 `bp-node-*` 的境外 TCP 443 握手失败（Uptime Kuma，
>   周期从本文 §7 的 300 s 改到 **60 s**）、`A2` 控制面不可达 **且** 有节点掉队（GCP 复合条件，
>   ⚠️ 必须用 `combiner: "AND"`，用 `AND_WITH_MATCHING_RESOURCE` 会得到一条**永不触发但看起来存在**的策略）、
>   `A3` 证书签发者 ≠ `O=Let's Encrypt`（**带外 VPS cron + msmtp 直接发信**）。
>   本表第 15 条「证书签发者变更 P0」就是 `A3`，但 0014 §14 把它的实现路径从
>   「Cloud Scheduler → `/internal/tasks/cert-check` → 日志指标 → GCP 告警」**整条挪到带外**
>   （**判定规则一字不改**，见本文 §8），理由是：检测「我们的前置基础设施是否被替换」不应依赖那套基础设施。
> - **B 级 11 条** GCP 策略，email#1 单通道。
> - **C 级不建策略**，只进看板与月巡检 —— 本表的第 4 条（API 429）被降到这一档，
>   理由是 `auth.go` 现有**应用级 429**（0014 §1.1 ⑧ 记为 7 处、§8.2 记为三处，**两处数字不一致，以 0014 正文为准**）。
>
> **0014 §14 对本文另有三处逐条修正，一并登记（本文今天不改）**：
>
> | 本文位置 | 现文 | 0014 的修正 |
> |---|---|---|
> | **§5 第 3 条** | API 5xx 用「比值 `> 1%`」 | 改**绝对计数** ≥5/10 min（理由：请求量算术下比值在低流量时毫无意义） |
> | **§5 第 4 条** | 「任何 429 都是异常」 | **降 C 级**（`auth.go` 的应用级 429 属正常） |
> | **§5 第 13 条** | 订阅 404 阈值 50 / 5 min | 改 **≥20/5 min**（由「单客户端最坏 5 次/5 min」的算术重定） |
> | 🔴 **§6.2 L371 + §7 L423** | `--matcher-content='"ok":true'` / `200 + "ok":true` | **全部改为纯文本 `ok`** —— 实现返回的是三个字节 `ok`（`openapi/openapi.yaml` L109「返回纯文本 `ok`。**不套信封**」+ `server.go` L44 `GetHealthz200TextResponse("ok")`），其中**不含子串 `"ok":true`**。照现文做，GCP check 与 Kuma **从第一天永久报红**。0014 §14 称这是它投入产出比最高的一条修正 |
>
> ⚠️ 同批的 [ADR 0011](../05-adr/0011-domain-blackout-detection.md) §14 **独立地把 §6.2 那条也判为「🔴 必修」**
> （否则整条判据永不启动），两份裁决在这一点上一致。
> 🔴 **本表今天不改写**：0014 状态是**提案，未批准**（2026-08-23）。
> **2026-09-02：0014 已批准，`setup-alerts.sh --apply` 已跑。** 上表 17 条对应到实建的是 B1–B9 + B11–B13
> 共 12 条（清单与 JSON 在 `infra/alerts/`），A2 / B10 被门挡着未建。上表第 15 条（证书签发者，P0）
> 的实现形态是**带内** Cloud Run Job + B 级告警，与 0014 §10.2 的带外要求有意偏离，理由见 0014 文档头。
...
>
> ⚠️ **`cert_check_failed`（握手失败 / 取不到证书）同样没有指标、没有告警。**
> 脚本这时会以 1 退出，但退出码只有跑它的那个作业看得见 —— 调度器接上去之前，
> 「域名整个连不上」这件事在告警面上是**静默**的。它和 `cert_expiring_soon` 是
> 本节欠的同一笔债：三个 event 里只有 `cert_issuer_bad` 那一个规划了指标。
...
- [ ] **SLO 与 error budget 未定义。** product-brief §8 有「主路径可用率 ≥99%（月）」，
      ...
      > **2026-08-29 补登落点：[ADR 0014](../05-adr/0014-slo-and-oncall.md)（roadmap B34，提案，未批准）。**
      > **「什么算可用」**：拆成三个 SLI，全部**按分钟**计不按请求计（0014 §3）——
      > `DP-a` 节点可服务性（境外探针，是真实可用率的**上界**）、`DP-b` 有效承载（唯一含中国侧的信号）、
      > `CP` 控制面可用性。
      > **「定几」**：0014 §4 裁决 **P1 期只发一个数字 —— `CP` = 98%/月**；
      > `DP-a` 与 `DP-b` 在 P1 期**只记账不承诺**，数据面的承诺改由事件型 `SLI-c`（封锁恢复时长）承接，
      > 先记账两个月再定线。98% 是「允许每月睡过去一次」的**算术翻译**，不是事故频率的乘积（§4.2）。
      > **「谁来算」**：0014 §3.6 —— 以带外 Uptime Kuma 的滚动 30 天数字为准，GCP uptime check 交叉验证，
      > 台账落 `docs/04-ops/slo-ledger.md`（**该文件尚不存在**）。
      > **product-brief §8 的 99% 不下调也不宣称**，只加一句限定（0014 §4.4）。
      > **error budget** 改**滚动 30 天**窗口，约束从四档改为两档 + 一个事件触发（0014 §5）。
      > 🔴 **本条不划掉**：0014 状态是**提案，未批准**（2026-08-23），
      > 且 §4.1 明说 `DP-a` 在 P1 期**故意没有 SLO**——「定义了」不等于「都有承诺」。
- [ ] **on-call 轮值、升级路径、告警静默流程未定义**（runbook §7 已列，至今仍是未解决）。
      本文定义了告警发到哪，没有定义**发给谁、多久没人应答怎么办**。
      > **2026-08-29 补登落点：[ADR 0014](../05-adr/0014-slo-and-oncall.md) §6–§8（roadmap B35，提案，未批准）。**
      > **裁决不假装有轮值**（§6.1 先承认约束：运维就是一个人）。
      > 升级路径由三段组成，**没有一段是「找另一个人」**（§6.2）：① 自动降级（控制面故障时数据面继续跑）
      > ② 显式延迟承诺 ③ **把内部用户变成 19:00–01:00 这 6 小时的第二告警通道**。
      > **「多久没人应答怎么办」**由 GCP 的 `renotifyInterval`（30 min–24 h）+ Kuma 的 5 分钟重发
      > **双层**提供，不单点押在那台 $5 VPS 上（§6.3）。
      > **响应时间承诺**见 §7：工作时间 **10:00–19:00 Asia/Shanghai**（不分工作日与周末），
      > A 级 30 分钟确认、B 级 4 小时响应，19:00 之后收到的 B 级顺延次日 10:00；
      > **无人值守窗口必须提前公告**，「尽快」不允许出现在任何对用户的承诺里。
      > **静默流程**见 §8.4：一律走 Snooze，必须带 ① 理由 ② 到期时间 ≤ 7 天并记
      > `docs/04-ops/alert-drill-ledger.md`（**该文件尚不存在**）；
      > **不想要的策略删掉，不要静默** —— 被永久静默的策略「在清单里仍然显示存在，这比不存在更危险」。
      > ⚠️ 0014 §12 另裁决了本文 §4 那条自我引用失效：A 级三通道，
      > 其中 email#2 是**只接 A 级的语义隔离箱**（§12.2）。
      > 🔴 **本条不划掉**：0014 状态是**提案，未批准**（2026-08-23）。
