...
> [ADR 0011](../05-adr/0011-domain-blackout-detection.md)（§4 与 §7 的域名失联，**提案，未批准**）、
> [ADR 0014](../05-adr/0014-slo-and-oncall.md) §14（关闭 §7 最后一条的值班/升级路径，**提案，未批准**）
> **2026-08-29 补登**：§7 加落点，§0–§6 的排障动作一处未改。
...
- [ ] 「域名被封」的**自动**检测未实现，当前流程全靠人工触发。
      ...
      > 自动信号由客户端内核的**直连探测腿**提供（0011 §3.2，境内、由内核定时器驱动），
      > 人工触发**不被取消**：0011 §11.1 明写触发链「必须有人工起点」。
      > 0011 §14 另给本文 §4 三条修正，一并登记：
      > ① §4 第 2 步「切换到域名池中的下一个域名，**更新 DNS**」被判为 ❌ **无效动作**（§5.1），
      > 改为「把该主机名标 `blocked` → 触发 §11.1 的链」；
      > ② §4 的触发条件「域名在多个运营商下均不可达」⚠️ **条件性保留** ——
      > 标注「本判据自 §3.2 的探测腿上线之日起生效，在此之前不可执行」，**不删**；
      > ③ ➕ 新增取证步骤（DNS 一致性 / TCP 握手 / TLS 阶段错误码 / 有无 RST / 是否整个后缀）——
      > 第一次真实封锁是回答 0011 §6.1 剩余不确定性的**唯一机会**。
...
- [ ] 值班制度、告警升级路径、on-call 轮值均未定义。
      > **2026-08-29 补登落点：[ADR 0014](../05-adr/0014-slo-and-oncall.md)（roadmap B35，提案，未批准）。**
      > 0014 §14 的「不改但要点名」一节写明**本文 §7 五条未解决项里，它关闭的正是这一条**，
      > 其余四条（探针机、域名被封自动检测、自动换 IP 脚本化、台账载体）不在 0014 范围内。
      > 裁决形态见 0014 §6：**不假装有轮值** —— 升级路径由三段组成，没有一段是「找另一个人」
      > （自动降级 + 显式延迟承诺 + 把内部用户变成 19:00–01:00 这 6 小时的第二告警通道）；
      > 响应时间承诺见 §7（工作时间 10:00–19:00 Asia/Shanghai，A 级 30 分钟确认、B 级 4 小时响应）；
      > 静默纪律见 §8.4（一律走 Snooze，必须带理由与 ≤7 天到期）。
