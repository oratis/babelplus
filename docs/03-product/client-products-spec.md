...
> **2026-09-02 后记**：用户指示把浏览器按 E0 之后的顺序排进 roadmap（[roadmap §5.2 2.E 与 §8](../00-overview/roadmap.md)）。
> go-to-market §4.2 的「不做」因此变成「排在扩展之后做」，进入门槛照 go-to-market §6 C 不变；§9 代价第 1 条原样保留。
...
**2026-09-02 排期落点**：整条链已按上图顺序排进 [roadmap §5.2 2.E](../00-overview/roadmap.md)，依赖图见 roadmap §8；
B1 的门是「扩展上架 30 天基线 + 『装不了扩展』的证据」，现在为零。
...
| **E5** 存活性实测 | 🔴 未做（需真机与两周） | — |
| **B1–B4** 浏览器 | 🔴 **未开工，已排期**（2026-09-02 用户指示）：roadmap 2.E 里排在 E4 之后，门是「扩展上架 30 天基线 + 『装不了扩展』的证据」；[go-to-market §4.2](go-to-market-plan.md) 的「不做」改为「排在扩展之后做」。签名证书、sing-box 自编译、Electron 跟版三项持续成本原样保留（§9 代价 1）；没有真机与证书时写出来的壳无法被验证，所以现在不写 | [roadmap §5.2 2.E](../00-overview/roadmap.md) |
