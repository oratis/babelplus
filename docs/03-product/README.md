...
| [design-system.md](design-system.md) | 设计方案 | **设计稿 v1，已落地到代码**（2026-09-05）——「控制台」视觉：五个界面一套色板（`web/shared/src/ui/tokens.css`）、零 webfont、等宽字承载数据、`Eyebrow` / `Led` / `Stat` / `Meter` 四个签名元件；官网 / 用户面板 / 后台已按此重做（691 用例全绿），扩展与桌面端只换色板、未在真机上看 |
