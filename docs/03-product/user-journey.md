...
> 🔴 **2026-09-01 追记：上面第 1、2 两条都已被后续裁决推翻，原文保留，落点写在这里**
> （roadmap **B59**；按 [docs/README §4](../README.md) 的规矩逐条交代）。
>
> **第 2 条「不是一单一地址」整条作废。**
> [ADR 0012 §3.6](../05-adr/0012-payment-gateway.md) 裁决为
> **一单一址、地址永不复用、归属只看 `to_address`、金额只用于判定 `paid` / `underpaid`**。
> 「小地址池 + 金额尾数唯一 + `+0.0001` 递增重试最多 100 次」这套机制**被取消**。
> 数据库侧的强制已在库：`pay_addresses.assigned_order_id` 是 `UNIQUE`
> （`0014_payments.up.sql:42`），另有 `UNIQUE (chain, address)` 与 `UNIQUE (chain, derivation_index)`。
> **代码也已按新裁决实现完并上线**（`92b65e0d5f9` / `3e18dd8b269`）。
> ⚠️ **ADR 0012 至今是「提案，未批准」** —— 所以准确表述是
> 「实现与 schema 已按新裁决走，而裁决本身还没批」，两边都悬着。
>
> **第 1 条「默认预选年付」也已不成立。**
> [pricing §3.2](pricing-and-plans.md) 逐格复算后把年付折扣从 75 折改到 85 折，
> 且**年付不随首发上架** —— 没有年付可预选。本条的链上成本论证（$2.13–4.31/笔）仍然成立，
> 但它现在支持的是「月付在链上是亏本的」这个事实，**不是「默认预选年付」这个界面决定**。
...
> 🔴 **2026-09-01 追记：卡点 3 的「我们能做什么」那一格是错的，不能照抄进收银台文案**（roadmap **B59**）。
> 「**尾数是订单识别码**」这句话的前提是 §6.1 第 2 条的小地址池 + 金额唯一性，
> 而那套机制已被 [ADR 0012 §3.6](../05-adr/0012-payment-gateway.md) 取消（一单一址、归属只看地址）。
> **在当前实现下，尾数不是识别码 —— 把它写进页面就是对用户撒谎。**
>
> **卡点 3 本身没有消失，但理由换了**：金额仍然要精确到 6 位定点（`amount_usdt6`），
> 因为它是判定 `paid` / `underpaid` 的分子。所以页面**仍要**一键复制精确金额，
> 文案应改成「少付会进入『还差 X』状态需要补足」这一类**后果导向**的解释，
> 而不是「尾数是识别码」这种**机制导向**的解释。
> ⚠️ 顺带：卡点 5（提币手续费从转出额里扣 → `underpaid` 头号成因）**完全不受影响**，
> 它在一单一址之下依然是第一位的失败模式，且现在有了明确的兜底路径
> （补足款走 [PR #25](https://github.com/oratis/babelplus/pull/25) 重写后的 paid/completed 三段分支）。
