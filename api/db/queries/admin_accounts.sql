...
WHERE u.deleted_at IS NULL
  ...
  AND (sqlc.narg(cursor_id)::bigint IS NULL OR u.id < sqlc.narg(cursor_id)::bigint)
ORDER BY u.id DESC
...
-- name: UpdateAdminUserEntitlement :one
WITH before AS (
  ...
  UPDATE users u SET
    expired_at   = coalesce(sqlc.narg(expired_at)::timestamptz, u.expired_at),
    device_limit = coalesce(sqlc.narg(device_limit)::integer,   u.device_limit),
    group_id     = coalesce(sqlc.narg(group_id)::bigint,        u.group_id),
    uuid         = coalesce(sqlc.narg(new_uuid)::uuid,          u.uuid),
    -- 总额 − 加油包分量；未传时保持原值。见上面的三步推理。
    transfer_enable_plan = coalesce(
      sqlc.narg(transfer_enable_total)::bigint - u.transfer_enable_pack,
...
WITH before AS (
  ...
  UPDATE users u SET
    banned        = true,
    banned_reason = sqlc.arg(reason)::text,
...
-- name: GetAdminBalanceAdjustAccounts :one
SELECT
...
-- name: ExportAdminUsersRows :many
SELECT
...
WHERE u.deleted_at IS NULL
  AND (sqlc.narg(banned)::boolean       IS NULL OR u.banned     = sqlc.narg(banned)::boolean)
  AND (sqlc.narg(plan_id)::bigint       IS NULL OR u.plan_id    = sqlc.narg(plan_id)::bigint)
  AND (sqlc.narg(group_id)::bigint      IS NULL OR u.group_id   = sqlc.narg(group_id)::bigint)
  ...
  AND (sqlc.narg(expired_before)::timestamptz IS NULL OR u.expired_at < sqlc.narg(expired_before)::timestamptz)
ORDER BY u.id
LIMIT sqlc.arg(page_limit)::integer;
...
FROM admin_users a
WHERE coalesce(sqlc.narg(include_disabled)::boolean, false) OR a.disabled_at IS NULL
ORDER BY a.id;
...
INSERT INTO admin_users (
  ...
  sqlc.arg(totp_secret_enc)::bytea,
  now(),
  sqlc.arg(role)::text,
  sqlc.narg(iap_subject)::text,
  sqlc.arg(perm_mark_order_paid)::boolean,
  sqlc.arg(perm_refund)::boolean,
  sqlc.arg(perm_adjust_balance)::boolean,
  sqlc.arg(perm_export_csv)::boolean
)
...
-- name: ResetAdminAccountTotp :one
WITH before AS (
  ...
  UPDATE admin_users a SET
    totp_secret_enc   = sqlc.arg(totp_secret_enc)::bytea,
...
-- name: ListAdminInvitesPage :many
SELECT
...
WHERE (sqlc.narg(owner_user_id)::bigint IS NULL OR ic.owner_user_id = sqlc.narg(owner_user_id)::bigint)
  AND (sqlc.narg(admin_seeded)::boolean IS NULL
       OR (sqlc.narg(admin_seeded)::boolean = (ic.owner_user_id IS NULL)))
  AND (sqlc.narg(cursor_id)::bigint IS NULL OR ic.id < sqlc.narg(cursor_id)::bigint)
ORDER BY ic.id DESC
LIMIT sqlc.arg(page_limit)::integer;

-- `?count=true` 才跑。WHERE 与上面逐字同形（同 §1.1 的理由）。
-- name: CountAdminInvites :one
...
-- name: CreateAdminInviteCodes :many
INSERT INTO invite_codes (code, owner_user_id, max_uses, expires_at, note)
SELECT c.code, NULL, sqlc.arg(max_uses)::integer,
       sqlc.narg(expires_at)::timestamptz, sqlc.narg(note)::text
FROM unnest(sqlc.arg(codes)::text[]) AS c(code)
...
-- name: AdjustAdminCommissionAmount :one
WITH before AS (
  SELECT c.id, c.order_id, c.inviter_id, c.invitee_id, c.amount, c.status,
         c.rate_bps, c.confirm_at, c.confirmed_at, c.created_at
  FROM commissions c
  WHERE c.id = sqlc.arg(commission_id)::bigint
  FOR UPDATE
), upd AS (
  UPDATE commissions c SET amount = c.amount + sqlc.arg(delta_amount)::bigint
  FROM before b
...
-- name: ListAdminAuditLogPage :many
SELECT
...
FROM audit_logs l
WHERE (sqlc.narg(admin_user_id)::bigint IS NULL OR l.admin_user_id = sqlc.narg(admin_user_id)::bigint)
  AND (sqlc.narg(action_like)::text IS NULL OR l.action ILIKE sqlc.narg(action_like)::text)
  ...
  AND (
    ...
    OR (l.created_at, l.id) < (sqlc.narg(cursor_at)::timestamptz, sqlc.narg(cursor_id)::bigint)
  )
...
-- name: UpdateAdminSettingsValues :many
WITH patch AS (
  SELECT k.key, v.val::jsonb AS value
  FROM unnest(sqlc.arg(setting_keys)::text[])   WITH ORDINALITY AS k(key, n)
  JOIN unnest(sqlc.arg(setting_values)::text[]) WITH ORDINALITY AS v(val, n) ON v.n = k.n
), before AS (
  ...
  UPDATE settings s SET
    value      = p.value,
    updated_by = sqlc.arg(admin_user_id)::bigint,
    updated_at = now()
  FROM patch p
  WHERE s.key = p.key
...
-- name: ListAdminStatByUser :many
SELECT s.stat_date, s.user_id, s.u AS upload_bytes, s.d AS download_bytes
FROM stat_user s
WHERE s.stat_date >= (sqlc.arg(from_at)::timestamptz AT TIME ZONE 'Asia/Shanghai')::date
  AND s.stat_date <= (sqlc.arg(to_at)::timestamptz   AT TIME ZONE 'Asia/Shanghai')::date
  AND (sqlc.narg(user_id)::bigint IS NULL OR s.user_id = sqlc.narg(user_id)::bigint)
...
-- name: ListAdminStatByServer :many
SELECT s.stat_date, s.server_id, s.u AS upload_bytes, s.d AS download_bytes
FROM stat_server s
WHERE s.stat_date >= (sqlc.arg(from_at)::timestamptz AT TIME ZONE 'Asia/Shanghai')::date
  AND s.stat_date <= (sqlc.arg(to_at)::timestamptz   AT TIME ZONE 'Asia/Shanghai')::date
  AND (sqlc.narg(server_id)::bigint IS NULL OR s.server_id = sqlc.narg(server_id)::bigint)
...
-- name: GetAdminDashboardOnlineUsers :one
SELECT
...
-- name: GetAdminDashboardNodes :one
...
-- name: GetAdminDashboardTrafficToday :one
SELECT
...
-- name: GetAdminDashboardRevenue :one
SELECT
...
-- name: GetAdminDashboardQueues :one
SELECT
...
-- name: ListAdminNoticesPage :many
SELECT
...
WHERE (
    sqlc.narg(cursor_at)::timestamptz IS NULL
    OR (n.created_at, n.id) < (sqlc.narg(cursor_at)::timestamptz, sqlc.narg(cursor_id)::bigint)
  )
ORDER BY n.created_at DESC, n.id DESC
LIMIT sqlc.arg(page_limit)::integer;

-- `?count=true` 才跑。公告是几十行的表，这条永远便宜。
-- name: CountAdminNotices :one
...
-- name: UpdateAdminNotice :one
WITH before AS (
  SELECT n.id, n.title, n.content_md, n.pinned, n.visible, n.starts_at, n.ends_at, n.sort_order
  FROM notices n
  WHERE n.id = sqlc.arg(notice_id)::bigint
  FOR UPDATE
), upd AS (
  UPDATE notices n SET
    title      = sqlc.arg(title)::text,
    content_md = sqlc.arg(content_md)::text,
    pinned     = coalesce(sqlc.narg(pinned)::boolean,        n.pinned),
    starts_at  = coalesce(sqlc.narg(starts_at)::timestamptz, n.starts_at),
    updated_at = now()
  FROM before b
  WHERE n.id = b.id
