...
WHERE s.user_id = sqlc.arg(user_id)::bigint
  AND s.stat_date > ((now() AT TIME ZONE 'Asia/Shanghai')::date - sqlc.arg(days)::int)
...
-- name: UpsertServerLoadSnapshot :exec
INSERT INTO server_online_state (
  ...
  sqlc.arg(server_id)::bigint,
  sqlc.arg(cpu_pct)::real,
  sqlc.narg(mem_total)::bigint,
  sqlc.narg(mem_used)::bigint,
  sqlc.narg(swap_total)::bigint,
  sqlc.narg(swap_used)::bigint,
  sqlc.narg(disk_total)::bigint,
  sqlc.narg(disk_used)::bigint,
  now()
)
...
-- name: RefreshServerOnlineUsers :exec
...
SELECT
  ...
  now()
FROM user_device_state d
WHERE d.server_id = sqlc.arg(server_id)::bigint
  AND d.last_seen_at > now() - interval '5 minutes'
ON CONFLICT (server_id) DO UPDATE SET
