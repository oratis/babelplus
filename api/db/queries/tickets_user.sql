...
-- name: ResolveTicketCategoryAndSLA :one
SELECT
  c.id               AS category_id,
...
CROSS JOIN users u
WHERE c.slug = sqlc.arg(category_slug)::text
...
SELECT
  ...
  sqlc.arg(priority)::ticket_priority,
  'web'::ticket_channel,
  jsonb_strip_nulls(jsonb_build_object(
    ...
    'client_reported',       sqlc.narg(client_reported)::jsonb
  )),
  now() + (sqlc.narg(first_response_minutes)::int * interval '1 minute'),
  now() + (sqlc.narg(resolution_minutes)::int * interval '1 minute')
FROM users u
...
FROM tickets t
LEFT JOIN ticket_categories c ON c.id = t.category_id
WHERE t.public_id = sqlc.arg(public_id)::text
