...
func (q *Queries) GetPlanForOrder(ctx context.Context, planID int64) (GetPlanForOrderRow, error) {
	row := q.db.QueryRow(ctx, getPlanForOrder, planID)
...
const listNoticesPage = `-- name: ListNoticesPage :many
...
WHERE n.visible = true
  ...
  AND (
    ...
    OR (n.pinned, n.created_at, n.id)
       < ($2::boolean, $1::timestamptz, $3::bigint)
  )
ORDER BY n.pinned DESC, n.created_at DESC, n.id DESC
LIMIT $4::integer
`

type ListNoticesPageParams struct {
...
type ListNoticesPageRow struct {
	ID          int64              `json:"id"`
...
func (q *Queries) ListNoticesPage(ctx context.Context, arg ListNoticesPageParams) ([]ListNoticesPageRow, error) {
	rows, err := q.db.Query(ctx, listNoticesPage,
		arg.CursorAt,
		arg.CursorPinned,
		arg.CursorID,
		arg.PageLimit,
	...
	defer rows.Close()
	items := []ListNoticesPageRow{}
	for rows.Next() {
		var i ListNoticesPageRow
		if err := rows.Scan(
			&i.ID,
			&i.Title,
			&i.ContentMd,
...
const listPlansForUser = `-- name: ListPlansForUser :many
...
type ListPlansForUserRow struct {
	ID                 int64       `json:"id"`
	Code               string      `json:"code"`
	Name               string      `json:"name"`
	Kind               string      `json:"kind"`
...
func (q *Queries) ListPlansForUser(ctx context.Context, userID int64) ([]ListPlansForUserRow, error) {
	rows, err := q.db.Query(ctx, listPlansForUser, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListPlansForUserRow{}
	for rows.Next() {
		var i ListPlansForUserRow
		if err := rows.Scan(
			&i.ID,
			&i.Code,
			&i.Name,
...
const verifyCouponForUser = `-- name: VerifyCouponForUser :one
...
SELECT
  ...
  (cardinality(c.scope_plan_ids) > 0
     ...
     AND NOT ($1::bigint = ANY (c.scope_plan_ids)))::boolean AS plan_out_of_scope,
  (cardinality(c.scope_periods) > 0
     AND $2::order_period IS NOT NULL
     AND NOT ($2::order_period = ANY (c.scope_periods)))::boolean AS period_out_of_scope,
  (cardinality(c.scope_plan_ids) > 0 AND $1::bigint IS NULL)::boolean AS plan_scope_unchecked,
  (cardinality(c.scope_periods) > 0 AND $2::order_period IS NULL)::boolean AS period_scope_unchecked,
  ...
  (SELECT count(*) FROM orders o2
    WHERE o2.user_id = $3::bigint
      AND o2.status IN ('paid','completed'))::bigint AS user_settled_order_count

FROM coupons c
WHERE upper(c.code) = upper($4::text)
`

type VerifyCouponForUserParams struct {
...
type VerifyCouponForUserRow struct {
	ID                    int64              `json:"id"`
	Code                  string             `json:"code"`
	Name                  string             `json:"name"`
	...
	PlanOutOfScope        bool               `json:"plan_out_of_scope"`
	PeriodOutOfScope      bool               `json:"period_out_of_scope"`
	PlanScopeUnchecked    bool               `json:"plan_scope_unchecked"`
	PeriodScopeUnchecked  bool               `json:"period_scope_unchecked"`
	UserUsedCount         int64              `json:"user_used_count"`
	UserSettledOrderCount int64              `json:"user_settled_order_count"`
}
...
func (q *Queries) VerifyCouponForUser(ctx context.Context, arg VerifyCouponForUserParams) (VerifyCouponForUserRow, error) {
	row := q.db.QueryRow(ctx, verifyCouponForUser,
		arg.PlanID,
		arg.Period,
		arg.UserID,
		arg.Code,
	)
	var i VerifyCouponForUserRow
	err := row.Scan(
		&i.ID,
		&i.Code,
		&i.Name,
		...
		&i.PlanOutOfScope,
		&i.PeriodOutOfScope,
		&i.PlanScopeUnchecked,
		&i.PeriodScopeUnchecked,
		&i.UserUsedCount,
		&i.UserSettledOrderCount,
	)
	return i, err
}
