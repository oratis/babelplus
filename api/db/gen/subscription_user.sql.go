...
const countActiveSubscriptionTokens = `-- name: CountActiveSubscriptionTokens :one
...
func (q *Queries) CountActiveSubscriptionTokens(ctx context.Context, userID int64) (int32, error) {
	row := q.db.QueryRow(ctx, countActiveSubscriptionTokens, userID)
	var active_tokens int32
	err := row.Scan(&active_tokens)
	return active_tokens, err
}

const getCnProxyModeRatio = `-- name: GetCnProxyModeRatio :one
SELECT
...
WHERE request_at > now() - $1::interval
`

type GetCnProxyModeRatioRow struct {
	ProxyUsers int64          `json:"proxy_users"`
	TotalUsers int64          `json:"total_users"`
	ProxyRatio pgtype.Numeric `json:"proxy_ratio"`
}
...
func (q *Queries) GetCnProxyModeRatio(ctx context.Context, lookback pgtype.Interval) (GetCnProxyModeRatioRow, error) {
	row := q.db.QueryRow(ctx, getCnProxyModeRatio, lookback)
	var i GetCnProxyModeRatioRow
	err := row.Scan(&i.ProxyUsers, &i.TotalUsers, &i.ProxyRatio)
	return i, err
}
...
func (q *Queries) GetUserSubscriptionSummary(ctx context.Context, userID int64) (GetUserSubscriptionSummaryRow, error) {
	row := q.db.QueryRow(ctx, getUserSubscriptionSummary, userID)
...
const insertUserSubscriptionFetchLog = `-- name: InsertUserSubscriptionFetchLog :one
...
type InsertUserSubscriptionFetchLogParams struct {
...
type InsertUserSubscriptionFetchLogRow struct {
	ID        int64              `json:"id"`
...
func (q *Queries) InsertUserSubscriptionFetchLog(ctx context.Context, arg InsertUserSubscriptionFetchLogParams) (InsertUserSubscriptionFetchLogRow, error) {
	row := q.db.QueryRow(ctx, insertUserSubscriptionFetchLog,
		arg.UserID,
		arg.TokenID,
		arg.RequestIp,
		arg.UserAgent,
	...
	var i InsertUserSubscriptionFetchLogRow
	err := row.Scan(&i.ID, &i.RequestAt)
	return i, err
}

const listUserNodesWithState = `-- name: ListUserNodesWithState :many
SELECT
  s.id,
  s.name,
  s.region,
...
type ListUserNodesWithStateRow struct {
	ID                 int64              `json:"id"`
	...
	SecondsSinceReport int64              `json:"seconds_since_report"`
}
...
func (q *Queries) ListUserNodesWithState(ctx context.Context, groupID int64) ([]ListUserNodesWithStateRow, error) {
	rows, err := q.db.Query(ctx, listUserNodesWithState, groupID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListUserNodesWithStateRow{}
	for rows.Next() {
		var i ListUserNodesWithStateRow
		if err := rows.Scan(
			&i.ID,
			&i.Name,
			&i.Region,
...
const listUserSubscriptionFetchLog = `-- name: ListUserSubscriptionFetchLog :many
SELECT
  f.id,
  f.request_at,
  f.request_ip,
...
WHERE f.user_id = $1::bigint
  AND (
    ...
    OR (f.request_at, f.id) < ($2::timestamptz, $3::bigint)
  )
ORDER BY f.request_at DESC, f.id DESC
LIMIT $4::int
`

type ListUserSubscriptionFetchLogParams struct {
	UserID    int64              `json:"user_id"`
...
type ListUserSubscriptionFetchLogRow struct {
	ID          int64              `json:"id"`
	RequestAt   pgtype.Timestamptz `json:"request_at"`
	RequestIp   netip.Addr         `json:"request_ip"`
	UserAgent   string             `json:"user_agent"`
...
func (q *Queries) ListUserSubscriptionFetchLog(ctx context.Context, arg ListUserSubscriptionFetchLogParams) ([]ListUserSubscriptionFetchLogRow, error) {
	rows, err := q.db.Query(ctx, listUserSubscriptionFetchLog,
		arg.UserID,
		arg.CursorAt,
		arg.CursorID,
		arg.PageLimit,
	...
	defer rows.Close()
	items := []ListUserSubscriptionFetchLogRow{}
	for rows.Next() {
		var i ListUserSubscriptionFetchLogRow
		if err := rows.Scan(
			&i.ID,
			&i.RequestAt,
			&i.RequestIp,
...
const listUserSubscriptionTokens = `-- name: ListUserSubscriptionTokens :many
...
type ListUserSubscriptionTokensRow struct {
	ID            int64              `json:"id"`
	Name          string             `json:"name"`
	TokenPrefix   string             `json:"token_prefix"`
...
func (q *Queries) ListUserSubscriptionTokens(ctx context.Context, userID int64) ([]ListUserSubscriptionTokensRow, error) {
	rows, err := q.db.Query(ctx, listUserSubscriptionTokens, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListUserSubscriptionTokensRow{}
	for rows.Next() {
		var i ListUserSubscriptionTokensRow
		if err := rows.Scan(
			&i.ID,
			&i.Name,
			&i.TokenPrefix,
...
const revokeAllUserSubscriptionTokens = `-- name: RevokeAllUserSubscriptionTokens :one
...
type RevokeAllUserSubscriptionTokensRow struct {
	SubRevokedAt pgtype.Timestamptz `json:"sub_revoked_at"`
...
func (q *Queries) RevokeAllUserSubscriptionTokens(ctx context.Context, userID int64) (RevokeAllUserSubscriptionTokensRow, error) {
	row := q.db.QueryRow(ctx, revokeAllUserSubscriptionTokens, userID)
	var i RevokeAllUserSubscriptionTokensRow
	err := row.Scan(&i.SubRevokedAt, &i.Revoked)
	return i, err
}
