...
const recordChainPayment = `-- name: RecordChainPayment :one
WITH prior AS (
  SELECT coalesce(sum(p.amount_usdt6), 0)::bigint AS received
  FROM payments p
  WHERE p.order_id = $3::bigint
...
SELECT $1::text, $2::text, 'scanner',
       $3::bigint, $4::bigint,
       $5::text, $6::text, $7::integer,
       $8::text, $9::text,
       $10::bigint,
       CASE
         WHEN NOT $11::boolean                THEN 'confirming'::payment_state
         WHEN $12::bigint IS NULL THEN 'confirming'::payment_state
         WHEN prior.received + $10::bigint
              >= $12::bigint    THEN 'paid'::payment_state
...
type RecordChainPaymentParams struct {
...
type RecordChainPaymentRow struct {
	ID      int64        `json:"id"`
...
func (q *Queries) RecordChainPayment(ctx context.Context, arg RecordChainPaymentParams) (RecordChainPaymentRow, error) {
	row := q.db.QueryRow(ctx, recordChainPayment,
		arg.Provider,
		arg.ExternalID,
		arg.OrderID,
		arg.UserID,
	...
	var i RecordChainPaymentRow
	err := row.Scan(&i.ID, &i.State, &i.OrderID)
	return i, err
}

const setOrderPayFromAddress = `-- name: SetOrderPayFromAddress :exec
UPDATE orders
SET pay_from_address = $1::text, updated_at = now()
WHERE id = $2::bigint AND pay_from_address IS NULL
`

type SetOrderPayFromAddressParams struct {
	FromAddress string `json:"from_address"`
	ID          int64  `json:"id"`
}
...
func (q *Queries) SetOrderPayFromAddress(ctx context.Context, arg SetOrderPayFromAddressParams) error {
	_, err := q.db.Exec(ctx, setOrderPayFromAddress, arg.FromAddress, arg.ID)
	return err
}
