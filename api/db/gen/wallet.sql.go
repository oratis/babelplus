...
WHERE (
  ...
  WHERE ic.owner_user_id = $2::bigint
...
type CreateUserInviteCodeParams struct {
	...
	MaxUnused   int32              `json:"max_unused"`
}
...
func (q *Queries) CreateUserInviteCode(ctx context.Context, arg CreateUserInviteCodeParams) (InviteCode, error) {
	row := q.db.QueryRow(ctx, createUserInviteCode,
		arg.Code,
		arg.OwnerUserID,
		arg.ExpiresAt,
		arg.Note,
...
const getCommissionTransferAccounts = `-- name: GetCommissionTransferAccounts :one
SELECT
...
type GetCommissionTransferAccountsRow struct {
...
func (q *Queries) GetCommissionTransferAccounts(ctx context.Context) (GetCommissionTransferAccountsRow, error) {
	row := q.db.QueryRow(ctx, getCommissionTransferAccounts)
	var i GetCommissionTransferAccountsRow
	err := row.Scan(&i.ExpenseAccountID, &i.WalletAccountID)
	return i, err
}

const getWalletOverview = `-- name: GetWalletOverview :one
...
SELECT
  ...
  coalesce((SELECT b.balance FROM wallet_balances b
             WHERE b.user_id = $1::bigint), 0)::bigint AS balance_cached
FROM wallet w CROSS JOIN commission cm
`

type GetWalletOverviewRow struct {
	BalanceLedger             int64 `json:"balance_ledger"`
	NonWithdrawableAmount     int64 `json:"non_withdrawable_amount"`
	WithdrawableAmount        int64 `json:"withdrawable_amount"`
	FromRefund                int64 `json:"from_refund"`
	FromCommission            int64 `json:"from_commission"`
	FromOrder                 int64 `json:"from_order"`
...
func (q *Queries) GetWalletOverview(ctx context.Context, userID int64) (GetWalletOverviewRow, error) {
	row := q.db.QueryRow(ctx, getWalletOverview, userID)
	var i GetWalletOverviewRow
	err := row.Scan(
		&i.BalanceLedger,
		&i.NonWithdrawableAmount,
		&i.WithdrawableAmount,
		&i.FromRefund,
		&i.FromCommission,
		&i.FromOrder,
		&i.CommissionPendingAmount,
		&i.CommissionAvailableAmount,
		&i.BalanceCached,
	)
	return i, err
}

const listUserCommissions = `-- name: ListUserCommissions :many
SELECT
  c.id,
...
type ListUserCommissionsParams struct {
	InviterID int64  `json:"inviter_id"`
	CursorID  *int64 `json:"cursor_id"`
	PageLimit int32  `json:"page_limit"`
}

type ListUserCommissionsRow struct {
	ID           int64              `json:"id"`
...
func (q *Queries) ListUserCommissions(ctx context.Context, arg ListUserCommissionsParams) ([]ListUserCommissionsRow, error) {
	rows, err := q.db.Query(ctx, listUserCommissions, arg.InviterID, arg.CursorID, arg.PageLimit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListUserCommissionsRow{}
	for rows.Next() {
		var i ListUserCommissionsRow
		if err := rows.Scan(
			&i.ID,
			&i.OrderTradeNo,
			&i.Amount,
...
const listUserInviteCodes = `-- name: ListUserInviteCodes :many
SELECT
  ic.id,
  ic.code,
  ic.max_uses,
  ...
  ic.created_at
FROM invite_codes ic
WHERE ic.owner_user_id = $1::bigint
ORDER BY ic.created_at DESC, ic.id DESC
`

type ListUserInviteCodesRow struct {
	ID        int64              `json:"id"`
	Code      string             `json:"code"`
...
func (q *Queries) ListUserInviteCodes(ctx context.Context, ownerUserID int64) ([]ListUserInviteCodesRow, error) {
	rows, err := q.db.Query(ctx, listUserInviteCodes, ownerUserID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListUserInviteCodesRow{}
	for rows.Next() {
		var i ListUserInviteCodesRow
		if err := rows.Scan(
			&i.ID,
			&i.Code,
			&i.MaxUses,
...
func (q *Queries) ListWalletTransactions(ctx context.Context, arg ListWalletTransactionsParams) ([]ListWalletTransactionsRow, error) {
	rows, err := q.db.Query(ctx, listWalletTransactions, arg.CursorID, arg.PageLimit, arg.UserID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const lockTransferableCommissions = `-- name: LockTransferableCommissions :many
...
type LockTransferableCommissionsRow struct {
	ID          int64              `json:"id"`
	Amount      int64              `json:"amount"`
	ConfirmedAt pgtype.Timestamptz `json:"confirmed_at"`
}
...
func (q *Queries) LockTransferableCommissions(ctx context.Context, inviterID int64) ([]LockTransferableCommissionsRow, error) {
	rows, err := q.db.Query(ctx, lockTransferableCommissions, inviterID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []LockTransferableCommissionsRow{}
	for rows.Next() {
		var i LockTransferableCommissionsRow
		if err := rows.Scan(&i.ID, &i.Amount, &i.ConfirmedAt); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
...
const markCommissionsTransferredBulk = `-- name: MarkCommissionsTransferredBulk :execrows
UPDATE commissions
SET status = 'transferred'
WHERE id = ANY($1::bigint[])
  AND inviter_id = $2::bigint
  AND status = 'confirmed'
`

type MarkCommissionsTransferredBulkParams struct {
	Ids       []int64 `json:"ids"`
	InviterID int64   `json:"inviter_id"`
}
...
func (q *Queries) MarkCommissionsTransferredBulk(ctx context.Context, arg MarkCommissionsTransferredBulkParams) (int64, error) {
	result, err := q.db.Exec(ctx, markCommissionsTransferredBulk, arg.Ids, arg.InviterID)
	if err != nil {
		return 0, err
	}
